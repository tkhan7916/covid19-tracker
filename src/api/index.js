...
export const fetchData = async (country) => {
    try {
        if(country) {
            ...
            return {
                confirmed: data[0].total_cases, 
                recovered: data[0].total_recovered, 
                deaths: data[0].total_deaths, 
...
export const fetchDailyData = async (country) => {
    try {
        ...
        if (country) {
            return data.map(({
                cases, recovered, deaths, last_update: date
            }) => ({
                confirmed: cases, recovered, deaths, date 
            }));
        } else {
            return data.map(({ 
                total_cases, total_recovered, total_deaths, last_update: date 
            }) => ({
